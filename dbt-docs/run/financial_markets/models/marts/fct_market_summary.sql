    ...
    create or replace table `irish-market-intelligence`.`FINANCIAL_MARKETS`.`fct_market_summary`
...
WITH latest_date AS (
    ...
    FROM `irish-market-intelligence`.`FINANCIAL_MARKETS`.`int_daily_returns`
    GROUP BY ticker
),
week52 AS (
...
ytd_price AS (
    ...
    FROM `irish-market-intelligence`.`FINANCIAL_MARKETS`.`int_daily_returns` r
